...
class TravelTimeMock(Common):
    ...
    count_by_transaction_id = {
        ...
        [config.MOCK_REQUEST_50_BIN, 50],
        "3bb4c6dd-9cad-4140-83c1-86d207fccb32":
        [config.MOCK_REQUEST_500_BIN, 500],
        "79d326c4-e29c-4c75-bedb-143c48dc717a":
    ...
    error_by_transaction_id = {
        "error500_invalid_grid_reference":
        config.MOCK_REQUEST_ERROR500_INVALID_GRID_REFERENCE,
    ...
    def post(self, transaction_id: str = "", service_count: int = -1):
        if service_count == 0:
            ...
            self.content = super().fetch_mock_proto_bin(
                self.response_path + config.MOCK_REQUEST_ERROR400_NO_SERVICES)
        elif service_count > 0 and service_count < 3001:
            if service_count in self.files_by_count:
                self.status_message = "MOCK Matched on count of " + str(
                    service_count)
            else:
            ...
            if transaction_id in self.count_by_transaction_id:
                ...
                self.content = super().fetch_mock_proto_bin(
                    self.response_path +
                    self.count_by_transaction_id[transaction_id][0])
                ...
                self.content = super().fetch_mock_proto_bin(
                    self.response_path +
                    self.error_by_transaction_id[transaction_id])
            else:
