...
docker-create-dockerfile: ### Create effective Dockerfile - mandatory: NAME; optional FILE=[Dockerfile name, defaults to Dockerfile]
	...
	sed -i " \
		...
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/elasticsearch:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/elasticsearch$(DOCKER_ARCH):$(DOCKER_LIBRARY_ELASTICSEARCH_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/nginx:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/nginx$(DOCKER_ARCH):$(DOCKER_LIBRARY_NGINX_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/node:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/node$(DOCKER_ARCH):$(DOCKER_LIBRARY_NODE_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/pipeline:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/pipeline$(DOCKER_ARCH):$(DOCKER_LIBRARY_PIPELINE_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/postgres:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/postgres$(DOCKER_ARCH):$(DOCKER_LIBRARY_POSTGRES_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/python-app:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/python-app$(DOCKER_ARCH):$(DOCKER_LIBRARY_PYTHON_APP_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/python:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/python$(DOCKER_ARCH):$(DOCKER_LIBRARY_PYTHON_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) $(DOCKER_LIBRARY_REGISTRY)/tools:latest#FROM $(DOCKER_LIBRARY_REGISTRY)/tools$(DOCKER_ARCH):$(DOCKER_LIBRARY_TOOLS_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) alpine:latest#FROM alpine$(DOCKER_ARCH):$(DOCKER_ALPINE_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) bitnami/elasticsearch:latest#FROM bitnami/elasticsearch$(DOCKER_ARCH):$(DOCKER_ELASTICSEARCH_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) docker:latest#FROM docker$(DOCKER_ARCH):$(DOCKER_DIND_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) gradle:latest#FROM gradle$(DOCKER_ARCH):$(DOCKER_GRADLE_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) maven:latest#FROM maven$(DOCKER_ARCH):$(DOCKER_MAVEN_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) nginx:latest#FROM nginx$(DOCKER_ARCH):$(DOCKER_NGINX_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) node:latest#FROM node$(DOCKER_ARCH):$(DOCKER_NODE_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) openjdk:latest#FROM openjdk$(DOCKER_ARCH):$(DOCKER_OPENJDK_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) postgres:latest#FROM postgres$(DOCKER_ARCH):$(DOCKER_POSTGRES_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) postman/newman:latest#FROM postman/newman$(DOCKER_ARCH):$(DOCKER_POSTMAN_NEWMAN_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) python:latest#FROM python$(DOCKER_ARCH):$(DOCKER_PYTHON_VERSION)#g; \
		s#FROM $(DOCKER_PLATFORM) rodolpheche/wiremock:latest#FROM rodolpheche/wiremock$(DOCKER_ARCH):$(DOCKER_WIREMOCK_VERSION)#g; \
